...
class LLMS_AT_Metabox {
	...
	private function hooks() {
		...
		add_action(
			'add_meta_boxes',
			array( $this, 'register_attendance_meta_boxes' )
		);
		...
		add_action(
			'save_post',
			array( $this, 'llms_attendance_add_query_string' ),
			100,
			3
		);
	...
	public function register_attendance_meta_boxes() {
		$disallow_attendance_text  = esc_html__( 'DisAllow Attendance ', 'llms-attendance' );
		$disallow_attendance_text  = apply_filters( 'llmsat_disallow_attendance_text', $disallow_attendance_text );
		$students_information_text = esc_html__( 'Students Attendance Information ', 'llms-attendance' );
		...
		add_meta_box(
			...
			array( $this, 'show_attendance_meta_box' ),
			'course',
			'side',
			'high'
		);
		...
		add_meta_box(
			...
			array( $this, 'show_student_listing_meta_box' ),
			'course',
			'advanced',
			'high'
		);
	...
	public function show_student_listing_meta_box() {
		...
		if ( $disallow == 'on' ) {
			echo '<div class="llmsat-error"><h2>' . esc_html__( 'Turn off the disallow attendance option to enlist enrolled students attendance information.', 'llms-attendance' ) . ' </h2></div>';
			return;
		}
	...
	public function show_attendance_meta_box() {
		...
		?>
		<div>
			<input type="checkbox" name="llmsatck1" 
			<?php
			if ( $disallow == true ) {
				?>
				checked="checked"<?php esc_html__( 'Disallow Attendance.', 'llms-attendance' ); } ?> />
		</div>
		<?php
	}
