...
class LLMS_At_Short_Code {
	...
	private function hooks() {
		...
		add_shortcode(
			...
			array( $this, 'display_top_attendant' )
		);
			...
			<ul id="">
				<li><span class=""><b><?php echo __( 'Invalid student ID or course ID :', 'llms-attendance' ); ?> </b></span>
					<ul class="llmsat-dicey">
						...
						<?php
						if ( ! $user ) {
							echo __( '<b>Student Name </b>: Student does not exist', 'llms-attendance' );} else {
						...
						<?php
						if ( $course_id ) {
							echo __( '<b>Course Name </b>: ' . get_the_title( $course_id ), 'llms-attendance' ); } else {
						...
						<li>
						<?php
						echo __( '<b>Attendance </b>: Invalid data ', 'llms-attendance' ); if ( $course_id && ! $has_access ) {
							?>
							</li>
